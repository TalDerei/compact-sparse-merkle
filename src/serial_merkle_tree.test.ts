import { assert } from "console";
import { MerkleTree, HashPath } from ".";
import { Sha256Hasher } from "./utils";

describe("merkle_tree", () => {
  const values: Buffer[] = [];
  const values2: Buffer[] = [];

  beforeAll(async () => {
    for (let i = 0; i < 2048; ++i) {
      const v = Buffer.alloc(64, 0);
      v.writeUInt32LE(i, 0);
      values[i] = v;
    }
  });

  it("should have correct empty tree root for depth 32", async () => {
    const tree = await MerkleTree.new(32);
    expect(tree.root.toString("hex")).toEqual(
      "c6f67e02e6e4e1bdefb994c6098953f34636ba2b6ca20a4721d2b26a886722ff",
    );
  });

  it("should have correct root for multiple batch updates with perfectly balanced subtrees", async () => {
    const tree = await MerkleTree.new(4);
    tree.insert(values.slice(0, 4));
    tree.insert(values.slice(4, 8));

    expect(tree.root).toEqual(
      Buffer.from(
        "0686691629308f433298fb60e09b4c25c65766492d49192ddbd4e3b9c56d5b3a",
        "hex",
      ),
    );
  });

  it("should have correct root for multiple batch updates with imbalanced subtrees", async () => {
    const tree = await MerkleTree.new(4);
    tree.insert(values.slice(0, 2));
    tree.insert(values.slice(2, 4));

    expect(tree.root).toEqual(
      Buffer.from(
        "b05876ac4e57afc5fa58e8501cadc8b51a5922778efb2d3dd4b8736e56f62218",
        "hex",
      ),
    );
  });

  it("should have correct root for multiple batch updates with imbalanced subtrees", async () => {
    const tree = await MerkleTree.new(4);
    tree.insert(values.slice(0, 2));
    tree.insert(values.slice(2, 4));
    tree.insert(values.slice(4, 6));

    expect(tree.root).toEqual(
      Buffer.from(
        "de7ad0e883bb84de4bef403f68a656cbe7edb901a94a84c385d25866e3973740",
        "hex",
      ),
    );
  });

  it("should have correct root for multiple batch updates with imbalanced subtrees", async () => {
    const tree = await MerkleTree.new(4);
    tree.insert(values.slice(0, 2));
    tree.insert(values.slice(2, 4));
    tree.insert(values.slice(4, 6));
    tree.insert(values.slice(6, 8));

    expect(tree.root).toEqual(
      Buffer.from(
        "0686691629308f433298fb60e09b4c25c65766492d49192ddbd4e3b9c56d5b3a",
        "hex",
      ),
    );
  });

  it("should have correct root", async () => {
    const hasher = new Sha256Hasher();
    const e00 = hasher.hash(values[0]);
    const e01 = hasher.hash(values[1]);
    const e02 = hasher.hash(values[2]);
    const e03 = hasher.hash(values[3]);
    const e10 = hasher.compress(e00, e01);
    const e11 = hasher.compress(e02, e03);
    const root = hasher.compress(e10, e11);

    const tree = await MerkleTree.new(2);
    tree.insert(values.slice(0, 4));

    let expected = new HashPath([
      [e00, e01],
      [e10, e11],
    ]);

    expect(await tree.requestHashPath(0)).toEqual(expected);
    expect(await tree.requestHashPath(1)).toEqual(expected);

    expected = new HashPath([
      [e02, e03],
      [e10, e11],
    ]);

    expect(await tree.requestHashPath(2)).toEqual(expected);
    expect(await tree.requestHashPath(3)).toEqual(expected);
    expect(tree.getRoot()).toEqual(root);

    expect(tree.root).toEqual(
      Buffer.from(
        "e645e6b5445483a358c4d15c1923c616a0e6884906b05c196d341ece93b2de42",
        "hex",
      ),
    );
  });

  it("should have correct results inserting 1024 values into 32 depth tree.", async () => {
    const tree = await MerkleTree.new(32);
    tree.insert(values.slice(0, 1024));

    expect(tree.root.toString("hex")).toBe(
      "26996bfcb0aaf96422aefdd789396a3f6c8a4fa6dccc73e55060e03e2a238db0",
    );

    const hashPath = await tree.requestHashPath(100);
    expect(hashPath.toBuffer().toString("hex")).toBe(
      "",
    );
  });

  it("should have the correct merkle path proof and verification for subtree", async () => {
    const tree = await MerkleTree.new(4);
    tree.insert(values.slice(0, 16));
    let merklePathProof = await tree.requestMerklePathProof(4);
    let verifyProof = await tree.verifyMerklePathProof(4, merklePathProof);

    expect(verifyProof.toString("hex")).toBe(tree.root.toString("hex"));
  });

  it("should have the correct merkle path proof and verification for full tree", async () => {
    const tree = await MerkleTree.new(32);
    tree.insert(values);
    let merklePathProof = await tree.requestMerklePathProof(100);
    let verifyProof = await tree.verifyMerklePathProof(100, merklePathProof);

    expect(verifyProof.toString("hex")).toBe(tree.root.toString("hex"));
  });

  it("should have the correct merkle path proof and verification for full tree with multiple batch insertions", async () => {
    const tree = await MerkleTree.new(32);
    tree.insert(values.slice(0, 1024));
    // TODO: why does this fail?
    // tree.insert(values.slice(1024, 2048));
    let merklePathProof = await tree.requestMerklePathProof(100);
    let verifyProof = await tree.verifyMerklePathProof(100, merklePathProof);

    expect(verifyProof.toString("hex")).toBe(tree.root.toString("hex"));
  });

  it("should have the correct updated value", async () => {
    const tree = await MerkleTree.new(32);
    tree.insert(values.slice(0, 1024));
    tree.update(0, values[1]);
  });
});
